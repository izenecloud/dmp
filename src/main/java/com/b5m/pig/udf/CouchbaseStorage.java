...
public final class CouchbaseStorage extends StoreFunc {
	...
	public String relToAbsPathForStoreLocation(String location, Path curDir)
			throws IOException {
		// no relative/absolute path conversion required since we are storing
		// into Couchbase
		return "/tmp";
	}
	...
	public void checkSchema(ResourceSchema schema) throws IOException {
		...
		if (fields.length != 2) {
			String message = String.format(
					"Expected input tuple of size 2, received %d",
					fields.length);
		...
		if (fields[0].getType() != DataType.CHARARRAY) {
			String message = String.format(
					"Expected first value to be chararray, received %s",
		...
		if (fields[1].getType() != DataType.TUPLE) {
			String message = String.format(
					"Expected second value to be a tuple, received %s",
		...
		Properties p = udfc.getUDFProperties(getClass(),
	 ...
	 * CouchbaseOutputFormat#getRecordWriter(). Hence we can safely suppress
	 * this warning.
	 */
	...
	public void prepareToWrite(RecordWriter writer) throws IOException {
		...
		if (log.isDebugEnabled())
			log.info("loaded schema from UDF context: " + schema);

		fieldSchema = schema.getFields();
		if (log.isDebugEnabled())
			log.debug("fieldSchema: " + Arrays.toString(fieldSchema));
	...
	public void putNext(Tuple tuple) throws IOException {
		try {
			...
			String value = jsonSerializer.toJson((Tuple) tuple.get(1),
					fieldSchema);
